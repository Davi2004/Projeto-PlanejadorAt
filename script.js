...
atividades = JSON.parse(localStorage.getItem("@listaAtividades")) || [];
...
const salvarAtividade = (event) => {
    ...
    const dadosFromulario = new FormData(event.target)

    const nome = dadosFromulario.get('atividade')
    const dia = dadosFromulario.get('dia')
    const hora = dadosFromulario.get('hora')
    const data = `${dia} ${hora}`
    ...
    if(atividadeExiste) {
        return alert(`🚨ALERTA!! Dia/Hora não disponíveis.🚨`)
    }
...
const criarDiasSelecao = () => {
    ...
    let diasSelecao = '<option value="Opção Selecionar" selected disabled> Selecione uma data </option>'
...
const criarHorasSelecao = () => {
    let horasDisponiveis = '<option value="Opção Selecionar" selected disabled> Selecione um horário </option>'
...
function salvarDados() {
    localStorage.setItem("@listaAtividades", JSON.stringify(atividades) )
}